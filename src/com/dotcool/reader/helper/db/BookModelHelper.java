...
public class BookModelHelper {
		      ...
		      + DBHelper.TABLE_NOVEL_BOOK + "(" + DBHelper.COLUMN_ID + " INTEGER PRIMARY KEY AUTOINCREMENT, "	// 0
				 				  + DBHelper.COLUMN_PAGE + " text not null, "						// 1
				  				  ...
				  				  + DBHelper.COLUMN_LAST_CHECK + " integer, "						// 4
				  				   + DBHelper.COLUMN_BOOK_ID + " integer, "                         //5
                                  + DBHelper.COLUMN_CHAPTER_ID + " integer, "                       //6
				  				  + DBHelper.COLUMN_ORDER + " integer);";							// 7
	...
	public static BookModel cursorToBookModel(Cursor cursor) {
		...
		book.setBookId(cursor.getInt(5));
		book.setChapter(cursor.getInt(6));
		book.setOrder(cursor.getInt(7));
		return book;
	}
	...
	public static BookModel insertBookModel(SQLiteDatabase db, BookModel book) {
		...
		cv2.put(DBHelper.COLUMN_BOOK_ID , book.getBookId());
	    cv2.put(DBHelper.COLUMN_CHAPTER_ID , book.getChapter());
